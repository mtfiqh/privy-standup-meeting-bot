...
describe('Test app/Calendar.js', ()=> {
    ...
    describe("#makeKeyboard", ()=>{
        ...
        it("True format should be has both 'text' and 'calback_data' properties!", ()=>{
            const format = cldr.makeKeyboard("prefix","text","action","args")
    ...
    describe("#createArgs", ()=>{
        it("All parameter should be integer", ()=>{
            try{
                ...
                assert.equal(error.message, "createArgs's params must be integer.")
            }
    ...
    describe("#parseArgs", ()=>{
        ...
        it("Args must be match to this pattern `/(([0-9]+)@){4,}[0-9]+/`.", ()=>{
            try{
                ...
                assert.equal(error.message, "Invalid Args format. Args must be match to this pattern `/(([0-9]+)@){4,}[0-9]+/`.")
            }
        })

        describe("Args '21@10@2019@0@0' is valid and : ", ()=>{
            const { day, month, year, row, col } = cldr.parseArgs("21@10@2019@0@0")
            it("Day should be 21", ()=>{ assert.equal(day, 21)})
            it("Month should be 10", ()=>{ assert.equal(month, 10)})
            it("Year should be 2019", ()=>{ assert.equal(year, 2019)})
            it("Row should be 0", ()=>{ assert.equal(row, 0)})
            it("Col should be 0", ()=>{ assert.equal(col, 0)})
        })
    ...
    describe("#isWeekend", ()=>{
        it("6 July 2019 and 7 July 2019 is weekend", ()=>{
            const bool = cldr.isWeekend(6,6,2019) && cldr.isWeekend(7,6,2019)
    ...
    describe("#countDay",()=>{
        it("22 July 2019 - 22 July 2019 = 1 day.",()=>{
            const day =  cldr.countDay(new Date(2019,6,22),new Date(2019,6,22))
            assert.equal(day, 1)
        })
        it("22 July 2019 - 26 July 2019 = 5 days.",()=>{
            const day =  cldr.countDay(new Date(2019,6,22),new Date(2019,6,26))
            assert.equal(day, 5)
        })
        it("26 July 2019 - 29 July 2019 = 4 days. (Weekend included)",()=>{
            const day =  cldr.countDay(new Date(2019,6,26),new Date(2019,6,29), false)
            assert.equal(day, 4)
        })
        it("26 July 2019 - 29 July 2019 = 2 days. (Weekend not included)",()=>{
            const day =  cldr.countDay(new Date(2019,6,26),new Date(2019,6,29))
            assert.equal(day, 2)
        })
        it("31 July 2019 - 1 Agustus 2019 = 2 days.",()=>{
            const day =  cldr.countDay(new Date(2019,6,31),new Date(2019,7,1))
            assert.equal(day, 2)
        })
        it("1 Agtustus 2019 - 31 July 2019 = 2 days.",()=>{
            const day =  cldr.countDay(new Date(2019,7,1),new Date(2019,6,31))
            assert.equal(day, 2)
        })
    })
